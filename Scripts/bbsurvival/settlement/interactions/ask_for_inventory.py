...
class BBSSettlementAskForInventoryInteraction(BBSocialSuperInteraction):
    """Ask another Sim to drop off their inventory to the asking Sim."""
    @classmethod
    ...
    def bbl_test(cls, interaction_sim_info: SimInfo, interaction_target_sim_info: SimInfo, interaction_context: InteractionContext, *args, **kwargs) -> BBTestResult:
        ...
        if not BBSSettlementUtils.is_head_of_settlement(interaction_sim_info):
            return BBTestResult.NONE
        if not BBSSettlementUtils.is_settlement_member(interaction_target_sim_info):
            return BBTestResult.NONE
        ...
        if source_sim_settlement_context is None or target_sim_settlement_context is None:
            cls.get_log().debug('No settlement context for either Sim.')
            return BBTestResult.NONE
        if source_sim_settlement_context is not target_sim_settlement_context:
            ...
            return BBTestResult.NONE
        if not BBSimInventoryUtils.has_any_inventory_items(interaction_target_sim_info):
            cls.get_log().debug('Source Sim has no items in their inventory.', source_sim=interaction_sim_info, target_sim=interaction_target_sim_info)
            return BBTestResult.NONE
        cls.get_log().debug('Sim is trying to ask for inventory.', sim=interaction_sim_info, target_sim=interaction_target_sim_info)
