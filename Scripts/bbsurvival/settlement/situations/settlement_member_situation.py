...
class _BBSSettlementMemberSituationBaseState(HasTunableFactory, AutoFactoryInit, SituationState):
    ...
    def __init__(self, *_, state_name: str = None, **__) -> None:
        super().__init__(*_, **__)
...
class BBSSettlementMemberSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {
        ...
        'working_state': _BBSSettlementMemberSituationState.TunableFactory(
            ...
            locked_args={'state_name': 'working'}
        ),
    }
    ...
    def _state_to_uid(cls, state_to_find: SituationState) -> int:
        ...
        for state_data in states:
            if getattr(state_data.state_type, 'state_name', None) == state_name or (state_data._factory is not None and getattr(state_data._factory, 'state_name', None) == state_name):
